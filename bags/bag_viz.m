...
for idx = 1:length(model_names)
  ...
  odom(idx).Data(:,:) = odom(idx).Data(:,:) + starting_offsets(idx,:);
end
...
for idx = 1:length(model_names)
    plot3(odom(idx).Data(:,1), odom(idx).Data(:,2), odom(idx).Data(:,3), ':');
    plot_handles(idx) = scatter3(odom(idx).Data(1,1), odom(idx).Data(1,2), odom(idx).Data(1,3), 50);
end
